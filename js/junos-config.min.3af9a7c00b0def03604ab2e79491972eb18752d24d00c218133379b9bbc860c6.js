...
`,e+=`    }
`));const a=document.getElementById("login-user").value,r=document.getElementById("login-password").value,y=document.getElementById("user-class").value;a&&r&&(e+=`    login {
`,e+=`        user ${a} {
`,e+=`            class ${y};
`,e+=`            authentication {
...
`,e+=`        }
`,e+=`    }
`);const j=document.getElementById("system-timezone").value;e+=`    time-zone ${j};
`;const n=document.getElementById("dns-servers").value;n&&(e+=`    name-server {
`,n.split(",").forEach(t=>{const n=t.trim();n&&(e+=`        ${n};
`)}),e+=`    }
`),e+=`}

`,e+=`interfaces {
`;const v=document.querySelectorAll(".interface-item");v.forEach((t,n)=>{let s=t.querySelector(`#interface-type-${n}`).value;if(s==="custom"){const o=document.getElementById(`custom-interface-name-${n}`),e=t.querySelector(`#custom-interface-value-${n}`).value;e?s=e:(console.warn("Custom interface selected but no value provided, using a default"),s="ge-0/0/0")}const a=t.querySelector(`#unit-number-${n}`).value||"0",o=t.querySelector(`#ip-address-${n}`).value,r=t.querySelector(`#prefix-length-${n}`).value||"24";e+=`    ${s} {
...
`}const m=document.getElementById("security-enabled");if(m&&m.checked){e+=`security {
...
`}const f=document.querySelectorAll(".filter-item");f.length>0&&(e+=`firewall {
`,f.forEach((t,n)=>{const s=t.querySelector(`#filter-name-${n}`).value,o=t.querySelector(`#filter-family-${n}`).value;if(s){e+=`    filter ${s} {
...
`);const s=document.getElementById("ntp-enabled");if(s&&s.checked){const t=document.getElementById("ntp-servers").value;t&&(e+=`system {
`,e+=`    ntp {
`,t.split(",").forEach(t=>{const n=t.trim();n&&(e+=`        server ${n};
`)}),e+=`    }
`,e+=`}

`)}const h=document.getElementById("dhcp-server-enabled");if(h&&h.checked){e+=`system {
`,e+=`    services {
`,e+=`        dhcp {
...
`}b.textContent=e,document.getElementById("config-output-container").style.display="block"}let e=1;const D=document.getElementById("add-filter"),u=document.getElementById("filters-list");if(D&&u){D.addEventListener("click",()=>{const t=document.createElement("div");t.className="filter-item",t.innerHTML=`
                <div class="input-group">
