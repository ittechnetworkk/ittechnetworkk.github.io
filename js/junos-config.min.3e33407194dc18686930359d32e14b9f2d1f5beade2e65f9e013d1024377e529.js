...
`,e+=`interfaces {
`;const v=document.querySelectorAll(".interface-item");v.forEach((t,n)=>{let s=t.querySelector(`#interface-type-${n}`).value;if(s==="custom"){const e=t.querySelector(`#custom-interface-value-${n}`).value;e?s=e:(console.warn("Custom interface selected but no value provided, using a default"),s="ge-0/0/0")}const a=t.querySelector(`#unit-number-${n}`).value||"0",o=t.querySelector(`#ip-address-${n}`).value,r=t.querySelector(`#prefix-length-${n}`).value||"24";e+=`    ${s} {
...
`,e+=`                peer-as ${o};
`;const i=t.querySelector(`#bgp-peer-export-policy-enabled-${n}`).checked;if(i){const s=t.querySelector(`#bgp-peer-export-policy-${n}`).value;s&&(e+=`                export ${s};
`)}e+=`            }
`}}),e+=`        }
`,e+=`    }
`,e+=`}
...
`}b.textContent=e,document.getElementById("config-output-container").style.display="block"}let e=1;const z=document.getElementById("add-filter"),h=document.getElementById("filters-list");if(z&&h){z.addEventListener("click",()=>{const t=document.createElement("div");t.className="filter-item",t.innerHTML=`
                <div class="input-group">
