let currentTab="enterprise",techniquesData={},threatGroupsData={},malwareData={},threatIntelligenceData={};const apiService=window.cybersecurityAPI||new CybersecurityAPIService,MITRE_API_BASE="https://attack.mitre.org/api/",VIRUSTOTAL_API_BASE="https://www.virustotal.com/vtapi/v2/",ALIENVAULT_API_BASE="https://otx.alienvault.com/api/v1/";document.addEventListener("DOMContentLoaded",function(){initializeFramework(),loadRealTimeData()});function initializeFramework(){const e=document.getElementById("techniquesList");e&&(e.innerHTML='<div class="loading">Loading MITRE ATT&CK data...</div>'),setupEventListeners(),loadCachedData()}async function loadRealTimeData(){try{showLoadingMessage("Loading real-time threat intelligence..."),await loadTechniquesFromAPI(),await loadThreatGroupsFromAPI(),await loadMalwareFromAPI(),await loadThreatIntelligence(),updateFrameworkUI(),showNotification("Real-time data loaded successfully!","success")}catch(e){console.error("Error loading real-time data:",e),showNotification("Some data could not be loaded. Using cached data.","info"),loadCachedData()}}async function loadTechniquesFromAPI(){try{techniquesData=await apiService.getMitreTechniques(),localStorage.setItem("mitreTechniques",JSON.stringify(techniquesData))}catch{console.warn("Could not load techniques from API, using cached data")}}async function loadThreatGroupsFromAPI(){try{threatGroupsData=await apiService.getMitreGroups(),localStorage.setItem("mitreThreatGroups",JSON.stringify(threatGroupsData))}catch{console.warn("Could not load threat groups from API, using cached data")}}async function loadMalwareFromAPI(){try{malwareData=await apiService.getMitreSoftware(),localStorage.setItem("mitreMalware",JSON.stringify(malwareData))}catch{console.warn("Could not load malware from API, using cached data")}}async function loadThreatIntelligence(){try{const e=await apiService.getThreatFoxMalware(10),t=await apiService.getURLhausPayloads(10),n=await apiService.getAlienVaultPulses(5);threatIntelligenceData={threatfox:e,urlhaus:t,alienvault:n},localStorage.setItem("mitreThreatIntelligence",JSON.stringify(threatIntelligenceData))}catch(e){console.warn("Could not load additional threat intelligence:",e)}}function processTechniquesData(e){const t={};return e.forEach(e=>{const n=e.tactic||"unknown";t[n]||(t[n]=[]),t[n].push({id:e.technique_id,name:e.technique_name,description:e.technique_description,url:e.technique_url,platforms:e.platforms||[],permissions:e.permissions||[],data_sources:e.data_sources||[]})}),t}function processThreatGroupsData(e){return e.map(e=>({id:e.group_id,name:e.group_name,description:e.group_description,url:e.group_url,techniques:e.techniques||[],aliases:e.aliases||[],country:e.country||"Unknown"}))}function processMalwareData(e){return e.map(e=>({id:e.software_id,name:e.software_name,description:e.software_description,url:e.software_url,techniques:e.techniques||[],platforms:e.platforms||[]}))}function loadCachedData(){const e=localStorage.getItem("mitreTechniques"),t=localStorage.getItem("mitreThreatGroups"),n=localStorage.getItem("mitreMalware");e&&(techniquesData=JSON.parse(e)),t&&(threatGroupsData=JSON.parse(t)),n&&(malwareData=JSON.parse(n)),updateFrameworkUI()}function updateFrameworkUI(){updateTechniqueExplorer(),updateThreatGroupsSection(),updateMalwareSection(),updateStatistics()}function updateTechniqueExplorer(){const e=document.getElementById("techniquesList");if(!e)return;e.innerHTML="<p>Select a tactic from the matrix above to view techniques.</p>"}function displayTechniques(e,t){const n=document.getElementById("techniquesList");if(!n)return;if(n.innerHTML="",e.length===0){n.innerHTML="<p>No techniques found for this tactic.</p>";return}const s=document.createElement("h3");s.textContent=`${t.charAt(0).toUpperCase()+t.slice(1).replace("-"," ")} Techniques`,n.appendChild(s),e.forEach(e=>{const t=document.createElement("div");t.className="technique-item enhanced",t.innerHTML=`
