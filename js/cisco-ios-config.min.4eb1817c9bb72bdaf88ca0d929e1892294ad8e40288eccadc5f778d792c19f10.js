document.addEventListener("DOMContentLoaded",()=>{const a=document.querySelectorAll(".tab-btn"),w=document.querySelectorAll(".tab-content"),y=document.querySelector('.tab-btn[data-tab="routing"]');y&&y.addEventListener("click",function(){w.forEach(e=>e.style.display="none"),document.getElementById("routing-content").style.display="block";const e=document.getElementById("static-routes-config");e&&(e.style.display="block"),a.forEach(e=>e.classList.remove("active")),this.classList.add("active")}),a.forEach(e=>{e.getAttribute("data-tab")!=="routing"&&e.addEventListener("click",()=>{a.forEach(e=>e.classList.remove("active")),e.classList.add("active"),w.forEach(e=>e.style.display="none");const t=e.getAttribute("data-tab");document.getElementById(`${t}-content`).style.display="block"})});const j=document.getElementById("ssh-enabled"),L=document.getElementById("ssh-config");j.addEventListener("change",()=>{L.style.display=j.checked?"block":"none"});const g=document.getElementById("acl-enabled"),D=document.getElementById("acl-config");g.addEventListener("change",()=>{D.style.display=g.checked?"block":"none"});const _=document.getElementById("ntp-enabled"),z=document.getElementById("ntp-config");_.addEventListener("change",()=>{z.style.display=_.checked?"block":"none"});const m=document.getElementById("dhcp-enabled"),O=document.getElementById("dhcp-config");m.addEventListener("change",()=>{O.style.display=m.checked?"block":"none"});const u=document.getElementById("syslog-enabled"),F=document.getElementById("syslog-config");u.addEventListener("change",()=>{F.style.display=u.checked?"block":"none"}),document.getElementById("interface-description-enabled-0").addEventListener("change",function(){document.getElementById("description-group-0").style.display=this.checked?"block":"none"});const l=document.getElementById("acl-type"),M=document.querySelectorAll(".extended-acl-fields");l.addEventListener("change",()=>{const e=l.value==="extended";M.forEach(t=>{t.style.display=e?"block":"none"})});const i=document.getElementById("acl-protocol-0"),A=document.querySelectorAll(".port-fields");i.addEventListener("change",()=>{const e=i.value==="tcp"||i.value==="udp";A.forEach(t=>{t.style.display=e?"block":"none"})});const o=document.getElementById("routing-protocol"),k=document.querySelectorAll(".protocol-config");o.addEventListener("change",()=>{k.forEach(e=>{e.style.display="none"});const e=o.value;if(e!=="none"){const t=document.getElementById(`${e}-config`);t&&(t.style.display="block")}}),o.value!=="static"&&(o.value="static"),document.getElementById("static-routes-config").style.display="block";let e=1;const C=document.getElementById("add-interface"),r=document.getElementById("interfaces-list");C.addEventListener("click",()=>{const t=document.createElement("div");t.className="interface-item",t.innerHTML=`
        ...
        `,r.appendChild(t),document.getElementById(`interface-description-enabled-${e}`).addEventListener("change",function(){document.getElementById(`description-group-${e}`).style.display=this.checked?"block":"none"});const n=t.querySelector(".remove-interface");n.addEventListener("click",()=>{r.removeChild(t)}),e++});let s=1;const x=document.getElementById("add-static-route"),c=document.getElementById("static-routes-list");x.addEventListener("click",()=>{const e=document.createElement("div");e.className="static-route-item",e.innerHTML=`
            <div class="input-group">
        ...
        `,c.appendChild(e);const t=e.querySelector(".remove-route");t.addEventListener("click",()=>{c.removeChild(e)}),s++});let n=1;const E=document.getElementById("add-acl-entry"),b=document.getElementById("acl-entries");E.addEventListener("click",()=>{const e=document.createElement("div");e.className="acl-entry",e.innerHTML=`
            <div class="input-group">
        ...
        `,b.appendChild(e);const t=e.querySelector(`#acl-protocol-${n}`),s=e.querySelector(".port-fields");t.addEventListener("change",()=>{const e=t.value==="tcp"||t.value==="udp";s.style.display=e?"block":"none"});const o=e.querySelector(".remove-acl-entry");o.addEventListener("click",()=>{b.removeChild(e)}),n++});let t=1;const S=document.getElementById("add-dhcp-pool"),d=document.getElementById("dhcp-pools");S.addEventListener("click",()=>{const e=document.createElement("div");e.className="dhcp-pool",e.innerHTML=`
            <div class="input-group">
        ...
        `,d.appendChild(e);const n=e.querySelector(".remove-dhcp-pool");n.addEventListener("click",()=>{d.removeChild(e)}),t++}),document.querySelector(".remove-interface").addEventListener("click",function(){if(document.querySelectorAll(".interface-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".interface-item");r.removeChild(e)}else alert("You need at least one interface.")});const h=document.querySelector(".remove-route");h&&h.addEventListener("click",function(){if(document.querySelectorAll(".static-route-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".static-route-item");c.removeChild(e)}else alert("You need at least one static route.")});const T=document.getElementById("generate-config"),p=document.getElementById("config-output"),f=document.getElementById("config-output-container");T.addEventListener("click",()=>{const e=P();p.textContent=e,f.style.display="block",f.scrollIntoView({behavior:"smooth"})});const N=document.getElementById("copy-config"),v=document.getElementById("copy-tooltip");N.addEventListener("click",()=>{H(p.textContent),v.style.display="block",setTimeout(()=>{v.style.display="none"},2e3)});const R=document.getElementById("reset-form");R.addEventListener("click",()=>{confirm("Are you sure you want to reset the form? All your configuration will be lost.")&&window.location.reload()});function P(){let e="";e+=`! Cisco IOS Configuration
