document.addEventListener("DOMContentLoaded",()=>{const b=document.querySelectorAll(".tab-btn"),k=document.querySelectorAll(".tab-content");b.forEach(e=>{e.addEventListener("click",()=>{b.forEach(e=>e.classList.remove("active")),e.classList.add("active"),k.forEach(e=>e.style.display="none");const t=e.getAttribute("data-tab");if(document.getElementById(`${t}-content`).style.display="block",t==="routing"){v.forEach(e=>{e.style.display="none"});const e=document.getElementById("routing-protocol").value;if(e!=="none"){const t=document.getElementById(`${e}-config`);t&&(t.style.display="block")}}})});const u=document.getElementById("ssh-enabled"),R=document.getElementById("ssh-config");u.addEventListener("change",()=>{R.style.display=u.checked?"block":"none"});const y=document.getElementById("acl-enabled"),N=document.getElementById("acl-config");y.addEventListener("change",()=>{N.style.display=y.checked?"block":"none"});const g=document.getElementById("ntp-enabled"),z=document.getElementById("ntp-config");g.addEventListener("change",()=>{z.style.display=g.checked?"block":"none"});const f=document.getElementById("dhcp-enabled"),E=document.getElementById("dhcp-config");f.addEventListener("change",()=>{E.style.display=f.checked?"block":"none"});const d=document.getElementById("syslog-enabled"),w=document.getElementById("syslog-config");d.addEventListener("change",()=>{w.style.display=d.checked?"block":"none"}),document.getElementById("interface-description-enabled-0").addEventListener("change",function(){document.getElementById("description-group-0").style.display=this.checked?"block":"none"});const i=document.getElementById("acl-type"),M=document.querySelectorAll(".extended-acl-fields");i.addEventListener("change",()=>{const e=i.value==="extended";M.forEach(t=>{t.style.display=e?"block":"none"})});const c=document.getElementById("acl-protocol-0"),S=document.querySelectorAll(".port-fields");c.addEventListener("change",()=>{const e=c.value==="tcp"||c.value==="udp";S.forEach(t=>{t.style.display=e?"block":"none"})});const o=document.getElementById("routing-protocol"),v=document.querySelectorAll(".protocol-config");o.addEventListener("change",()=>{v.forEach(e=>{e.style.display="none"});const e=o.value;if(e!=="none"){const t=document.getElementById(`${e}-config`);t&&(t.style.display="block")}}),o.value!=="static"&&(o.value="static"),document.getElementById("static-routes-config").style.display="block";let e=1;const T=document.getElementById("add-interface"),a=document.getElementById("interfaces-list");T.addEventListener("click",()=>{const t=document.createElement("div");t.className="interface-item",t.innerHTML=`
        ...
        `,a.appendChild(t),document.getElementById(`interface-description-enabled-${e}`).addEventListener("change",function(){document.getElementById(`description-group-${e}`).style.display=this.checked?"block":"none"});const n=t.querySelector(".remove-interface");n.addEventListener("click",()=>{a.removeChild(t)}),e++});let s=1;const O=document.getElementById("add-static-route"),r=document.getElementById("static-routes-list");O.addEventListener("click",()=>{const e=document.createElement("div");e.className="static-route-item",e.innerHTML=`
            <div class="input-group">
        ...
        `,r.appendChild(e);const t=e.querySelector(".remove-route");t.addEventListener("click",()=>{r.removeChild(e)}),s++});let n=1;const C=document.getElementById("add-acl-entry"),_=document.getElementById("acl-entries");C.addEventListener("click",()=>{const e=document.createElement("div");e.className="acl-entry",e.innerHTML=`
            <div class="input-group">
        ...
        `,_.appendChild(e);const t=e.querySelector(`#acl-protocol-${n}`),s=e.querySelector(".port-fields");t.addEventListener("change",()=>{const e=t.value==="tcp"||t.value==="udp";s.style.display=e?"block":"none"});const o=e.querySelector(".remove-acl-entry");o.addEventListener("click",()=>{_.removeChild(e)}),n++});let t=1;const A=document.getElementById("add-dhcp-pool"),p=document.getElementById("dhcp-pools");A.addEventListener("click",()=>{const e=document.createElement("div");e.className="dhcp-pool",e.innerHTML=`
            <div class="input-group">
        ...
        `,p.appendChild(e);const n=e.querySelector(".remove-dhcp-pool");n.addEventListener("click",()=>{p.removeChild(e)}),t++}),document.querySelector(".remove-interface").addEventListener("click",function(){if(document.querySelectorAll(".interface-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".interface-item");a.removeChild(e)}else alert("You need at least one interface.")});const m=document.querySelector(".remove-route");m&&m.addEventListener("click",function(){if(document.querySelectorAll(".static-route-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".static-route-item");r.removeChild(e)}else alert("You need at least one static route.")});const F=document.getElementById("generate-config"),h=document.getElementById("config-output"),l=document.getElementById("config-output-container");F.addEventListener("click",()=>{const e=x();h.textContent=e,l.style.display="block",l.scrollIntoView({behavior:"smooth"})});const D=document.getElementById("copy-config"),j=document.getElementById("copy-tooltip");D.addEventListener("click",()=>{P(h.textContent),j.style.display="block",setTimeout(()=>{j.style.display="none"},2e3)});const L=document.getElementById("reset-form");L.addEventListener("click",()=>{confirm("Are you sure you want to reset the form? All your configuration will be lost.")&&window.location.reload()});function x(){let e="";e+=`! Cisco IOS Configuration
