...
`}const g=document.querySelectorAll(".filter-item");g.length>0&&(e+=`firewall {
`,g.forEach((t,n)=>{const s=t.querySelector(`#filter-name-${n}`).value,o=t.querySelector(`#filter-family-${n}`).value;if(s){e+=`    filter ${s} {
...
`,t+=`        dhcp {
`;const e=document.querySelectorAll(".dhcp-pool");e.forEach((e,n)=>{const s=e.querySelector(`#dhcp-pool-name-${n}`).value,o=e.querySelector(`#dhcp-network-${n}`).value,i=e.querySelector(`#dhcp-range-${n}`).value,a=e.querySelector(`#dhcp-router-${n}`).value,r=e.querySelector(`#dhcp-dns-${n}`).value,c=e.querySelector(`#dhcp-lease-${n}`).value;s&&o&&i&&(t+=`            pool ${s} {
`,t+=`                subnet ${o};
`,t+=`                address-range ${i};
`,a&&(t+=`                router ${a};
`),r&&(t+=`                name-server ${r};
`),c&&(t+=`                maximum-lease-time ${c};
`),t+=`            }
`)}),t+=`        }
`,t+=`    }
