const COMMON_PORTS=[21,22,23,25,53,80,110,143,443,465,587,993,995,3306,3389,5432,8080];async function getIPInfo(){try{const t=document.querySelectorAll(".loading");t.forEach(e=>e.classList.add("loading"));const n=await fetch("https://ipwhois.app/json/"),e=await n.json();if(e&&e.success){try{const n=await fetch("https://api-ipv6.ip.sb/ip"),t=await n.text(),s=document.getElementById("ip-address");s.innerHTML=`
                    <div class="ip-row">
                        <span>IPv4: ${e.ip}</span>
                        <button class="copy-btn" onclick="copyToClipboard('${e.ip}')">
                            <i class="fas fa-copy"></i>
                        </button>
                    </div>
                    <div class="ip-row">
                        <span>IPv6: ${t.trim()}</span>
                        <button class="copy-btn" onclick="copyToClipboard('${t.trim()}')">
                            <i class="fas fa-copy"></i>
                        </button>
                    </div>
                `}catch{console.log("IPv6 not available, trying alternative service");try{const n=await fetch("https://v6.ident.me/"),t=await n.text(),s=document.getElementById("ip-address");s.innerHTML=`
                        <div class="ip-row">
                            <span>IPv4: ${e.ip}</span>
                            <button class="copy-btn" onclick="copyToClipboard('${e.ip}')">
                                <i class="fas fa-copy"></i>
...
`);t.textContent=s||"No headers found",t.className="result-box success"}catch{t.textContent="Error checking headers",t.className="result-box error"}}function setErrorState(){const n=["location","isp","country","region","city","timezone","coordinates","postal","currency"],e=document.getElementById("ip-address");e&&(e.innerHTML=`
            <div class="ip-row">
        ...
        `),n.forEach(e=>{const t=document.getElementById(e);t&&(t.textContent="Not available",t.classList.remove("loading"))});const t=document.getElementById("country-flag");t.src="",t.alt="Flag not available"}function copyToClipboard(e){navigator.clipboard.writeText(e).then(()=>{const e=document.createElement("div");e.className="copy-notification",e.textContent="Copied!",document.body.appendChild(e),setTimeout(()=>{e.remove()},2e3)}).catch(e=>{console.error("Failed to copy:",e)})}document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("refresh-ip");e.addEventListener("click",getIPInfo),document.querySelectorAll(".copy-btn").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.copy,n=document.getElementById(t).textContent;copyToClipboard(n)})});const t=document.getElementById("start-ping");t.addEventListener("click",()=>{const e=document.getElementById("ping-host").value.trim();e&&pingHost(e)});const n=document.getElementById("start-dns");n.addEventListener("click",()=>{const e=document.getElementById("dns-host").value.trim();e&&dnsLookup(e)});const s=document.getElementById("check-headers");s.addEventListener("click",()=>{const e=document.getElementById("headers-url").value.trim();e&&checkHeaders(e)}),setTimeout(getIPInfo,100)}),window.addEventListener("load",getIPInfo);const style=document.createElement("style");style.textContent=`
.ip-row {
    display: flex;
    align-items: center;
    gap: 10px;
