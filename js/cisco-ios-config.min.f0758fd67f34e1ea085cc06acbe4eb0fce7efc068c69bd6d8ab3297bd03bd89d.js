document.addEventListener("DOMContentLoaded",()=>{const m=document.querySelectorAll(".tab-btn"),O=document.querySelectorAll(".tab-content");m.forEach(e=>{e.addEventListener("click",()=>{m.forEach(e=>e.classList.remove("active")),e.classList.add("active"),O.forEach(e=>e.style.display="none");const t=e.getAttribute("data-tab");document.getElementById(`${t}-content`).style.display="block"})});const u=document.getElementById("ssh-enabled"),L=document.getElementById("ssh-config");u.addEventListener("change",()=>{L.style.display=u.checked?"block":"none"});const j=document.getElementById("acl-enabled"),T=document.getElementById("acl-config");j.addEventListener("change",()=>{T.style.display=j.checked?"block":"none"});const c=document.getElementById("ntp-enabled"),F=document.getElementById("ntp-config");c.addEventListener("change",()=>{F.style.display=c.checked?"block":"none"});const h=document.getElementById("dhcp-enabled"),S=document.getElementById("dhcp-config");h.addEventListener("change",()=>{S.style.display=h.checked?"block":"none"});const d=document.getElementById("syslog-enabled"),y=document.getElementById("syslog-config");d.addEventListener("change",()=>{y.style.display=d.checked?"block":"none"}),document.getElementById("interface-description-enabled-0").addEventListener("change",function(){document.getElementById("description-group-0").style.display=this.checked?"block":"none"});const o=document.getElementById("acl-type"),k=document.querySelectorAll(".extended-acl-fields");o.addEventListener("change",()=>{const e=o.value==="extended";k.forEach(t=>{t.style.display=e?"block":"none"})});const r=document.getElementById("acl-protocol-0"),E=document.querySelectorAll(".port-fields");r.addEventListener("change",()=>{const e=r.value==="tcp"||r.value==="udp";E.forEach(t=>{t.style.display=e?"block":"none"})});const a=document.getElementById("routing-protocol"),x=document.querySelectorAll(".protocol-config");a.addEventListener("change",()=>{x.forEach(e=>{e.style.display="none"});const e=a.value;e!=="none"&&(document.getElementById(`${e}-config`).style.display="block")}),a.value==="static"&&(document.getElementById("static-routes-config").style.display="block");let e=1;const D=document.getElementById("add-interface"),i=document.getElementById("interfaces-list");D.addEventListener("click",()=>{const t=document.createElement("div");t.className="interface-item",t.innerHTML=`
        ...
        `,l.appendChild(e);const n=e.querySelector(".remove-dhcp-pool");n.addEventListener("click",()=>{l.removeChild(e)}),t++}),document.querySelector(".remove-interface").addEventListener("click",function(){if(document.querySelectorAll(".interface-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".interface-item");i.removeChild(e)}else alert("You need at least one interface.")});const M=document.getElementById("generate-config"),f=document.getElementById("config-output"),g=document.getElementById("config-output-container");M.addEventListener("click",()=>{const e=w();f.textContent=e,g.style.display="block",g.scrollIntoView({behavior:"smooth"})});const z=document.getElementById("copy-config"),v=document.getElementById("copy-tooltip");z.addEventListener("click",()=>{R(f.textContent),v.style.display="block",setTimeout(()=>{v.style.display="none"},2e3)});const N=document.getElementById("reset-form");N.addEventListener("click",()=>{confirm("Are you sure you want to reset the form? All your configuration will be lost.")&&window.location.reload()});function w(){let e="";e+=`! Cisco IOS Configuration
