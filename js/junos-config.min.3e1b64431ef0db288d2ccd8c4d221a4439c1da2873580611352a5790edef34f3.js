document.addEventListener("DOMContentLoaded",()=>{const z=document.querySelectorAll(".tab-btn"),oe=document.querySelectorAll(".tab-content");z.forEach(e=>{e.addEventListener("click",()=>{z.forEach(e=>e.classList.remove("active")),e.classList.add("active"),oe.forEach(e=>e.style.display="none");const t=e.getAttribute("data-tab");document.getElementById(`${t}-content`).style.display="block"})});const ie=[{checkbox:"interface-description-enabled",section:"description-group"},{checkbox:"ipv6-enabled",section:"ipv6-config"},{checkbox:"vlan-enabled",section:"vlan-config"},{checkbox:"firewall-filter-enabled",section:"firewall-filter-config"},{checkbox:"zones-enabled",section:"zones-config"},{checkbox:"ntp-enabled",section:"ntp-config"},{checkbox:"ntp-source-interface-enabled",section:"ntp-source-interface-group"},{checkbox:"dhcp-server-enabled",section:"dhcp-server-config"},{checkbox:"syslog-enabled",section:"syslog-config"},{checkbox:"snmp-enabled",section:"snmp-config"},{checkbox:"policy-options-enabled",section:"policy-options-config"},{checkbox:"cos-enabled",section:"cos-config"},{checkbox:"routing-policy-prefix-list-enabled",section:"routing-policy-prefix-list-group"},{checkbox:"bgp-peer-import-policy-enabled",section:"bgp-peer-import-policy"},{checkbox:"bgp-peer-export-policy-enabled",section:"bgp-peer-export-policy"}];ie.forEach(e=>{const t=document.getElementById(e.checkbox+"-0");if(t){const n=document.getElementById(e.section+"-0");n&&t.addEventListener("change",()=>{n.style.display=t.checked?"block":"none"})}const n=document.getElementById(e.checkbox);if(n){const t=document.getElementById(e.section);t&&n.addEventListener("change",()=>{t.style.display=n.checked?"block":"none"})}});const E=document.getElementById("routing-protocol"),ae=document.querySelectorAll(".protocol-config");E&&E.addEventListener("change",()=>{ae.forEach(e=>{e.style.display="none"});const e=E.value;e!=="none"&&(document.getElementById(`${e}-config`).style.display="block")});const w=document.getElementById("term-protocol-0"),S=document.getElementById("port-fields-0");w&&S&&w.addEventListener("change",()=>{w.value==="tcp"||w.value==="udp"?S.style.display="block":S.style.display="none"});let t=1;const T=document.getElementById("add-interface"),_=document.getElementById("interfaces-list");if(T&&_){T.addEventListener("click",()=>{const e=document.createElement("div");e.className="interface-item",e.innerHTML=`
            ...
            `,_.appendChild(e);const n=document.getElementById(`ipv6-enabled-${t}`);n.addEventListener("change",function(){const e=document.getElementById(`ipv6-config-${t}`);e.style.display=this.checked?"block":"none"});const s=document.getElementById(`interface-description-enabled-${t}`);s.addEventListener("change",function(){const e=document.getElementById(`description-group-${t}`);e.style.display=this.checked?"block":"none"});const o=document.getElementById(`vlan-enabled-${t}`);o.addEventListener("change",function(){const e=document.getElementById(`vlan-config-${t}`);e.style.display=this.checked?"block":"none"});const i=e.querySelector(".remove-interface");i.addEventListener("click",function(){document.querySelectorAll(".interface-item").length>1?_.removeChild(e):alert("You need at least one interface.")});const a=document.getElementById(`interface-type-${t}`);a.addEventListener("change",function(){const e=document.getElementById(`custom-interface-name-${t}`);e.style.display=this.value==="custom"?"block":"none"}),updateInterfaceSelects(),t++});const e=document.querySelector(".remove-interface");e&&e.addEventListener("click",function(){if(document.querySelectorAll(".interface-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".interface-item");_.removeChild(e)}else alert("You need at least one interface.")})}let c=1;const G=document.getElementById("add-static-route"),g=document.getElementById("static-routes-list");if(G&&g){G.addEventListener("click",()=>{const e=document.createElement("div");e.className="static-route-item",e.innerHTML=`
                <div class="input-group">
            ...
            `,b.appendChild(e);const t=e.querySelector(`#bgp-peer-import-policy-enabled-${n}`),o=e.querySelector(`#bgp-peer-import-policy-${n}`);t&&o&&t.addEventListener("change",()=>{o.style.display=t.checked?"block":"none"});const s=e.querySelector(`#bgp-peer-export-policy-enabled-${n}`),i=e.querySelector(`#bgp-peer-export-policy-${n}`);s&&i&&s.addEventListener("change",()=>{i.style.display=s.checked?"block":"none"});const a=e.querySelector(".remove-bgp-neighbor");a.addEventListener("click",function(){document.querySelectorAll(".bgp-neighbor-item").length>1?b.removeChild(e):alert("You need at least one BGP neighbor.")}),n++});const s=document.querySelector(".remove-bgp-neighbor");s&&s.addEventListener("click",function(){if(document.querySelectorAll(".bgp-neighbor-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".bgp-neighbor-item");b.removeChild(e)}else alert("You need at least one BGP neighbor.")});const e=document.getElementById("bgp-peer-import-policy-enabled-0"),o=document.getElementById("bgp-peer-import-policy-0");e&&o&&e.addEventListener("change",()=>{o.style.display=e.checked?"block":"none"});const t=document.getElementById("bgp-peer-export-policy-enabled-0"),i=document.getElementById("bgp-peer-export-policy-0");t&&i&&t.addEventListener("change",()=>{i.style.display=t.checked?"block":"none"})}const W=document.getElementById("generate-config"),y=document.getElementById("config-output"),k=document.getElementById("config-output-container");W&&y&&k&&W.addEventListener("click",()=>{const e=re();y.textContent=e,k.style.display="block",k.scrollIntoView({behavior:"smooth"})});const V=document.getElementById("reset-form");V&&V.addEventListener("click",()=>{confirm("Are you sure you want to reset the form? All your configuration will be lost.")&&window.location.reload()});const F=document.getElementById("copy-config"),C=document.getElementById("copy-tooltip");F&&y&&C&&F.addEventListener("click",()=>{J(y.textContent),C.style.display="block",setTimeout(()=>{C.style.display="none"},2e3)});function J(e){const t=document.createElement("textarea");t.value=e,document.body.appendChild(t),t.select(),document.execCommand("copy"),document.body.removeChild(t)}function re(){const r=document.getElementById("config-output"),c=document.getElementById("hostname").value||"router",t=document.getElementById("domain-name").value;let e="";e+=`system {
`,e+=`    host-name ${c};
`,t&&(e+=`    domain-name ${t};
`);const n=document.getElementById("root-password").value,s=document.getElementById("use-encrypted-password").checked;n&&(s?(e+=`    root-authentication {
`,e+=`        encrypted-password "$9$XXXXXXXXXXXXXXXXXXXXXX"; # Reference only, replace with real encrypted password
`,e+=`    }
`):(e+=`    root-authentication {
`,e+=`        plain-text-password-value "${n}";
`,e+=`    }
`));const o=document.getElementById("login-user").value,i=document.getElementById("login-password").value,l=document.getElementById("user-class").value;o&&i&&(e+=`    login {
`,e+=`        user ${o} {
`,e+=`            class ${l};
`,e+=`            authentication {
`,s?e+=`                encrypted-password "$9$XXXXXXXXXXXXXXXXXXXXXX"; # Reference only, replace with real encrypted password
`:e+=`                plain-text-password-value "${i}";
`,e+=`            }
`,e+=`        }
`,e+=`    }
`);const d=document.getElementById("system-timezone").value;e+=`    time-zone ${d};
`;const a=document.getElementById("dns-servers").value;a&&(e+=`    name-server {
`,a.split(",").forEach(t=>{const n=t.trim();n&&(e+=`        ${n};
`)}),e+=`    }
`),e+=`}

`,e+=`interfaces {
`;const u=document.querySelectorAll(".interface-item");u.forEach((t,n)=>{let s=t.querySelector(`#interface-type-${n}`).value;if(s==="custom"){const e=t.querySelector(`#custom-interface-value-${n}`).value;e?s=e:(console.warn("Custom interface selected but no value provided, using a default"),s="ge-0/0/0")}const a=t.querySelector(`#unit-number-${n}`).value||"0",o=t.querySelector(`#ip-address-${n}`).value,r=t.querySelector(`#prefix-length-${n}`).value||"24";e+=`    ${s} {
...
`,r.textContent=e,document.getElementById("config-output-container").style.display="block"}let e=1;const N=document.getElementById("add-filter"),h=document.getElementById("filters-list");if(N&&h){N.addEventListener("click",()=>{const t=document.createElement("div");t.className="filter-item",t.innerHTML=`
                <div class="input-group">
            ...
            `,x.appendChild(e);const t=e.querySelector(".remove-zone");t&&t.addEventListener("click",function(){document.querySelectorAll(".zone-item").length>1?x.removeChild(e):alert("You need at least one security zone.")}),i++});const e=document.querySelector(".remove-zone");e&&e.addEventListener("click",function(){if(document.querySelectorAll(".zone-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".zone-item");x.removeChild(e)}else alert("You need at least one security zone.")})}let o=1;const D=document.getElementById("add-policy"),f=document.getElementById("policies-list");if(D&&f){D.addEventListener("click",()=>{const e=document.createElement("div");e.className="policy-item",e.innerHTML=`
                <div class="input-group">
            ...
            `,f.appendChild(e);const t=e.querySelector(".remove-policy");t&&t.addEventListener("click",function(){document.querySelectorAll(".policy-item").length>1?f.removeChild(e):alert("You need at least one security policy.")}),o++});const e=document.querySelector(".remove-policy");e&&e.addEventListener("click",function(){if(document.querySelectorAll(".policy-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".policy-item");f.removeChild(e)}else alert("You need at least one security policy.")})}let r=1;const R=document.getElementById("add-prefix-list"),m=document.getElementById("prefix-lists");if(R&&m){R.addEventListener("click",()=>{const e=document.createElement("div");e.className="prefix-list-item",e.innerHTML=`
                <div class="input-group">
            ...
            `,m.appendChild(e);const n=e.querySelector(".add-prefix-entry"),t=e.querySelector(`#prefix-entries-${r}`);if(n&&t){let e=1;n.addEventListener("click",()=>{const n=document.createElement("div");n.className="prefix-entry",n.innerHTML=`
                        <div class="input-group">
        ...
        `,n.appendChild(t);const o=document.getElementById(`ipv6-enabled-${e}`);o.addEventListener("change",function(){const t=document.getElementById(`ipv6-config-${e}`);t.style.display=this.checked?"block":"none"});const i=document.getElementById(`interface-description-enabled-${e}`);i.addEventListener("change",function(){const t=document.getElementById(`description-group-${e}`);t.style.display=this.checked?"block":"none"});const a=document.getElementById(`vlan-enabled-${e}`);a.addEventListener("change",function(){const t=document.getElementById(`vlan-config-${e}`);t.style.display=this.checked?"block":"none"});const r=t.querySelector(".remove-interface");r.addEventListener("click",function(){document.querySelectorAll(".interface-item").length>1?n.removeChild(t):alert("You need at least one interface.")});const c=document.getElementById(`interface-type-${e}`);c.addEventListener("change",function(){const t=document.getElementById(`custom-interface-name-${e}`);t.style.display=this.value==="custom"?"block":"none"}),updateInterfaceSelects()}let B=document.getElementById("add-interface");B&&B.addEventListener("click",ne);const Z=document.getElementById("ipv6-enabled-0");Z&&Z.addEventListener("change",function(){const e=document.getElementById("ipv6-config-0");e.style.display=this.checked?"block":"none"});const q=document.getElementById("interface-description-enabled-0");q&&q.addEventListener("change",function(){const e=document.getElementById("description-group-0");e.style.display=this.checked?"block":"none"});const H=document.getElementById("vlan-enabled-0");H&&H.addEventListener("change",function(){const e=document.getElementById("vlan-config-0");e.style.display=this.checked?"block":"none"});const A=document.getElementById("interface-type-0");A&&(A.classList.add("interface-type-select"),A.addEventListener("change",function(){const e=document.getElementById("custom-interface-name-0");e&&(e.style.display=this.value==="custom"?"block":"none")}))})