...
`,e+=`interfaces {
`;const y=document.querySelectorAll(".interface-item");y.forEach((t,n)=>{let s=t.querySelector(`#interface-type-${n}`).value;if(s==="custom"){const o=document.getElementById(`custom-interface-name-${n}`),e=t.querySelector(`#custom-interface-value-${n}`).value;e?s=e:(console.warn("Custom interface selected but no value provided, using a default"),s="ge-0/0/0")}const a=t.querySelector(`#unit-number-${n}`).value||"0",o=t.querySelector(`#ip-address-${n}`).value,r=t.querySelector(`#prefix-length-${n}`).value||"24";e+=`    ${s} {
...
`)}const b=document.getElementById("dhcp-server-enabled");if(b&&b.checked){e+=`system {
`,e+=`    services {
`,e+=`        dhcp {
...
`}const f=document.getElementById("syslog-enabled");if(f&&f.checked){const t=document.getElementById("syslog-server").value,n=document.getElementById("syslog-facility").value,s=document.getElementById("syslog-severity").value;t&&(e+=`system {
`,e+=`    syslog {
`,e+=`        host ${t} {
`,e+=`            facility ${n};
`,e+=`            level ${s};
...
`)}const u=document.getElementById("snmp-enabled");if(u&&u.checked){const t=document.getElementById("snmp-community").value,n=document.getElementById("snmp-location").value,s=document.getElementById("snmp-contact").value,o=document.getElementById("snmp-clients").value;t&&(e+=`system {
`,e+=`    snmp {
`,n&&(e+=`        location "${n}";
`),s&&(e+=`        contact "${s}";
`),e+=`        community ${t} {
