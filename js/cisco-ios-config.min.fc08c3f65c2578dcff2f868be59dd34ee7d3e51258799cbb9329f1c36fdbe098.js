document.addEventListener("DOMContentLoaded",()=>{const o=document.querySelectorAll(".tab-btn"),O=document.querySelectorAll(".tab-content"),w=document.querySelector('.tab-btn[data-tab="routing"]');w&&w.addEventListener("click",function(){O.forEach(e=>e.style.display="none"),document.getElementById("routing-content").style.display="block";const e=document.getElementById("static-routes-config");e&&(e.style.display="block"),o.forEach(e=>e.classList.remove("active")),this.classList.add("active")}),o.forEach(e=>{e.getAttribute("data-tab")!=="routing"&&e.addEventListener("click",()=>{o.forEach(e=>e.classList.remove("active")),e.classList.add("active"),O.forEach(e=>e.style.display="none");const t=e.getAttribute("data-tab");document.getElementById(`${t}-content`).style.display="block"})});const _=document.getElementById("ssh-enabled"),L=document.getElementById("ssh-config");_.addEventListener("change",()=>{L.style.display=_.checked?"block":"none"});const u=document.getElementById("acl-enabled"),D=document.getElementById("acl-config");u.addEventListener("change",()=>{D.style.display=u.checked?"block":"none"});const p=document.getElementById("ntp-enabled"),z=document.getElementById("ntp-config");p.addEventListener("change",()=>{z.style.display=p.checked?"block":"none"});const h=document.getElementById("dhcp-enabled"),x=document.getElementById("dhcp-config");h.addEventListener("change",()=>{x.style.display=h.checked?"block":"none"});const v=document.getElementById("syslog-enabled"),F=document.getElementById("syslog-config");v.addEventListener("change",()=>{F.style.display=v.checked?"block":"none"}),document.getElementById("interface-description-enabled-0").addEventListener("change",function(){document.getElementById("description-group-0").style.display=this.checked?"block":"none"});const c=document.getElementById("acl-type"),M=document.querySelectorAll(".extended-acl-fields");c.addEventListener("change",()=>{const e=c.value==="extended";M.forEach(t=>{t.style.display=e?"block":"none"})});const r=document.getElementById("acl-protocol-0"),A=document.querySelectorAll(".port-fields");r.addEventListener("change",()=>{const e=r.value==="tcp"||r.value==="udp";A.forEach(t=>{t.style.display=e?"block":"none"})});const a=document.getElementById("routing-protocol"),b=document.querySelectorAll(".protocol-config");a&&(a.value="static",b.forEach(e=>{e.id==="static-routes-config"?e.style.display="block":e.style.display="none"}),a.addEventListener("change",function(){b.forEach(e=>{e.style.display="none"});const e=this.value;if(e!=="none"){const t=document.getElementById(`${e}-config`);t&&(t.style.display="block")}})),document.getElementById("static-routes-config").style.display="block";let e=1;const k=document.getElementById("add-interface"),i=document.getElementById("interfaces-list");k.addEventListener("click",()=>{const t=document.createElement("div");t.className="interface-item",t.innerHTML=`
        ...
        `,i.appendChild(t),document.getElementById(`interface-description-enabled-${e}`).addEventListener("change",function(){document.getElementById(`description-group-${e}`).style.display=this.checked?"block":"none"});const n=t.querySelector(".remove-interface");n.addEventListener("click",()=>{i.removeChild(t)}),e++});let s=1;const C=document.getElementById("add-static-route"),l=document.getElementById("static-routes-list");C.addEventListener("click",()=>{const e=document.createElement("div");e.className="static-route-item",e.innerHTML=`
            <div class="input-group">
        ...
        `,l.appendChild(e);const t=e.querySelector(".remove-route");t.addEventListener("click",()=>{l.removeChild(e)}),s++});let n=1;const E=document.getElementById("add-acl-entry"),y=document.getElementById("acl-entries");E.addEventListener("click",()=>{const e=document.createElement("div");e.className="acl-entry",e.innerHTML=`
            <div class="input-group">
            ...
            <div class="extended-acl-fields" style="display: ${c.value==="extended"?"block":"none"}">
                <div class="input-group">
        ...
        `,y.appendChild(e);const t=e.querySelector(`#acl-protocol-${n}`),s=e.querySelector(".port-fields");t.addEventListener("change",()=>{const e=t.value==="tcp"||t.value==="udp";s.style.display=e?"block":"none"});const o=e.querySelector(".remove-acl-entry");o.addEventListener("click",()=>{y.removeChild(e)}),n++});let t=1;const S=document.getElementById("add-dhcp-pool"),f=document.getElementById("dhcp-pools");S.addEventListener("click",()=>{const e=document.createElement("div");e.className="dhcp-pool",e.innerHTML=`
            <div class="input-group">
        ...
        `,f.appendChild(e);const n=e.querySelector(".remove-dhcp-pool");n.addEventListener("click",()=>{f.removeChild(e)}),t++}),document.querySelector(".remove-interface").addEventListener("click",function(){if(document.querySelectorAll(".interface-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".interface-item");i.removeChild(e)}else alert("You need at least one interface.")});const d=document.querySelector(".remove-route");d&&d.addEventListener("click",function(){if(document.querySelectorAll(".static-route-item").length>1){const t=this.getAttribute("data-id"),e=this.closest(".static-route-item");l.removeChild(e)}else alert("You need at least one static route.")});const T=document.getElementById("generate-config"),m=document.getElementById("config-output"),g=document.getElementById("config-output-container");T.addEventListener("click",()=>{const e=P();m.textContent=e,g.style.display="block",g.scrollIntoView({behavior:"smooth"})});const N=document.getElementById("copy-config"),j=document.getElementById("copy-tooltip");N.addEventListener("click",()=>{H(m.textContent),j.style.display="block",setTimeout(()=>{j.style.display="none"},2e3)});const R=document.getElementById("reset-form");R.addEventListener("click",()=>{confirm("Are you sure you want to reset the form? All your configuration will be lost.")&&window.location.reload()});function P(){let e="";e+=`! Cisco IOS Configuration
...
`,e}function H(e){const t=document.createElement("textarea");t.value=e,document.body.appendChild(t),t.select(),document.execCommand("copy"),document.body.removeChild(t)}})