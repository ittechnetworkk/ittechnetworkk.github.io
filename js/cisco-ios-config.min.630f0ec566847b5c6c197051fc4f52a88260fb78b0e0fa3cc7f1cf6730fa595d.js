...
`;const r=document.getElementById("hostname").value||"Router1",n=document.getElementById("domain-name").value;e+=`hostname ${r}
`,n&&(e+=`ip domain-name ${n}
`);const s=document.getElementById("enable-secret").value;s&&(e+=`enable secret ${s}
`);const o=document.getElementById("banner-motd").value;o&&(e+=`banner motd #
${o}
#
`);const i=document.getElementById("console-password").value;i&&(e+=`
...
`,e+=` password ${i}
`,e+=` login
`);const a=document.getElementById("vty-password").value;if(a&&(e+=`
! VTY configuration
`,e+=`line vty 0 4
`,e+=` logging synchronous
`,e+=` exec-timeout 30 0
`,e+=` password ${a}
`,e+=` login
`),document.getElementById("ssh-enabled").checked){const s=document.getElementById("ssh-version").value,t=document.getElementById("ssh-username").value,n=document.getElementById("ssh-password").value;e+=`
! SSH configuration
`,e+=`ip ssh version ${s}
`,t&&n&&(e+=`username ${t} privilege 15 secret ${n}
`,e+=`
line vty 0 4
...
! DHCP configuration
`,t.forEach(t=>{const n=t.querySelector(".remove-dhcp-pool").getAttribute("data-id"),s=document.getElementById(`dhcp-name-${n}`).value,o=document.getElementById(`dhcp-network-${n}`).value,i=document.getElementById(`dhcp-mask-${n}`).value,a=document.getElementById(`dhcp-gateway-${n}`).value,r=document.getElementById(`dhcp-dns-${n}`).value,c=document.getElementById(`dhcp-domain-${n}`).value,l=document.getElementById(`dhcp-lease-${n}`).value;s&&o&&i&&(e+=`ip dhcp pool ${s}
`,e+=` network ${o} ${i}
